...
#include <unistd.h>
// #define _GNU_SOURCE maybe include MAP_STACK in mmap if things are exploding
// running on server
...
/*
 * lwp_wrap - helper function from which thread functions are called from.
 * necessary to guarantee return value is passed into lwp_exit
 *
...
tid_t lwp_create(lwpfun func, void *arg) {
  ...
  // TODO: Make sure memory allocated for the stack is also munmap'd at some
  // point in the future (probably in exit or wait)
  void *lwp_stack = mmap(NULL, stack_size, PROT_READ | PROT_WRITE,
  ...
  unsigned long *top =
      (unsigned long *)lwp_stack +
      (stack_size / (sizeof(unsigned long))); // calculate top of stack
